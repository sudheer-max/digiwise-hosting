...
export default function DockerView() {
  ...
  return (
    <ResourcePage
      title="Pods"
      subtitle="Inspect running pods across your Kubernetes cluster."
      ...
      empty={<div className="text-center py-14 text-sm text-slate-400">No pods found.</div>}
      children={(rows) => {
        ...
        return (
          <div className="space-y-4">
            ...
            <PageTable
              ...
              columns={[
                { key: 'name', label: 'Pod', render: (p) => <span className="font-mono text-xs font-bold text-slate-900">{p.metadata?.name || p.name}</span> },
                { key: 'namespace', label: 'Namespace', render: (p) => <span className="text-xs text-slate-600">{p.metadata?.namespace || p.namespace || '—'}</span> },
            ...
            {selected && (
              <DetailPanel title={<span className="font-mono">{selected.metadata?.name || selected.name}</span>} icon={<Box className="w-4 h-4 text-[#00459c]" />} onClose={() => setSelected(null)}>
                ...
                <KV label="Namespace" value={selected.metadata?.namespace || selected.namespace} />
                ...
                <KV label="Pod IP" value={selected.status?.podIP || selected.podIP || '—'} />
