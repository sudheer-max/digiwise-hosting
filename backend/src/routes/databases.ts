    ...
    try {
      ...
      if (type === 'mongodb') {
        ...
        const script = [
          ...
          `for d in ${dumpDir}/*/; do n=$(basename "$d"); if [ "$n" != "${vars.databaseName}" ]; then mv "$d" ${dumpDir}/${vars.databaseName}; break; fi; done`,
          `mongorestore --uri='${targetUri}' --dir=${dumpDir}/${vars.databaseName} --gzip --drop 2>&1`,
          `rm -rf ${dumpDir}`,
        ].join(' && ');
