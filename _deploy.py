...
print('=== Upload backend builds.ts ===', flush=True)
client = connect()
sftp = client.open_sftp()
sftp.put(os.path.join(LOCAL, 'backend', 'src', 'services', 'build.ts'), f'{REMOTE}/backend/src/services/build.ts')
sftp.put(os.path.join(LOCAL, 'backend', 'src', 'routes', 'apps.ts'), f'{REMOTE}/backend/src/routes/apps.ts')
sftp.close()
client.close()
...
print('\n=== Import frontend to K3s ===', flush=True)
client = connect()
...
print('\n=== Verify ===', flush=True)
client = connect()
run(client, 'curl -sk -o /dev/null -w "Frontend: %{http_code}\n" https://digiwisesoftech.com/auth/login')
run(client, 'curl -sk -o /dev/null -w "Backend: %{http_code}\n" https://api.digiwisesoftech.com/health')
client.close()
